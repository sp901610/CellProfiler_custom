...
class DefaultCoordinate(cellprofiler_core.setting.Coordinates):
    """
    ...
    def test_valid(self, pipeline):
        ...
        if len(values) < 2:
            raise cellprofiler_core.setting.ValidationError("X and Y values must be separated by a comma", self)
        if len(values) > 2:
            raise cellprofiler_core.setting.ValidationError("Only two values allowed", self)
        for value in values:
            try:
                int(value.strip())
            except ValueError:
                raise cellprofiler_core.setting.ValidationError("{} is not an integer".format(value), self)


class ActiveContourModel(ImageSegmentation):
    ...
    def create_settings(self):
        ...
        self.method = cellprofiler_core.setting.choice.Choice(
        ...
        self.iterations = cellprofiler_core.setting.text.Integer(
            text="Iterations",
            value=20
        )

        # Only shared by PDE and GEODESIC
        self.threshold = cellprofiler_core.setting.text.Float(
            text="Threshold",
            value=0
        )

        # Pre-processing settings
        self.pde_alpha = cellprofiler_core.setting.text.Float(
            text="Alpha",
            value=0.2
        )

        self.advanced_settings = cellprofiler_core.setting.Binary(
            text="Advanced settings for PDE method",
            value=False
        )

        self.phi_bound = cellprofiler_core.setting.text.Float(
            text="Phi bound",
            value=1.2
        )

        self.pre_threshold = cellprofiler_core.setting.text.Float(
        ...
        self.cfl_factor = cellprofiler_core.setting.text.Float(
            text="CFL condition maintenance factor",
            value=0.45
        )

        self.sdf_smoothing = cellprofiler_core.setting.text.Float(
            text="SDF smoothing factor",
            value=0.5
        )

        # Shared morph settings
        self.alpha = cellprofiler_core.setting.text.Float(
            text="Alpha",
            value=100.0
        )

        self.sigma = cellprofiler_core.setting.text.Float(
            text="Sigma",
            value=5.0
        )

        self.level_set = cellprofiler_core.setting.choice.Choice(
            text="Initial level set",
            choices=[LEVEL_SET_CIRCLE, LEVEL_SET_CHECKERBOARD],
            value=LEVEL_SET_CIRCLE
        )

        self.adv_level_set = cellprofiler_core.setting.Binary(
        ...
        self.level_set_iterative = cellprofiler_core.setting.Binary(
        ...
        self.level_set_size = cellprofiler_core.setting.text.Float(
            text="Level set size",
            value=-1.,
            minval=0.,
            doc=textwrap.dedent("""
        ...
        self.level_set_iterative_sizes = cellprofiler_core.setting.text.Text(
        ...
        self.smoothing = cellprofiler_core.setting.text.Integer(
            text="Smoothing",
            value=1,
            minval=0
        )
        ...
        # Geodesic settings
        self.balloon = cellprofiler_core.setting.text.Float(
            text="Balloon force",
            value=0.
        )

        # Morph Chan-Vese settings
        self.outer_weight = cellprofiler_core.setting.text.Float(
            text="Outer region weight",
            value=1.
        )

        self.inner_weight = cellprofiler_core.setting.text.Float(
    ...
    def validate_module(self, pipeline):
        ...
        try:
            ...
            raise cellprofiler_core.setting.ValidationError("'{}' is not a suitable list of values for {} level set!"
        ...
        if self.adv_level_set.value:
            ...
            if not ((center_x == center_y == -1) or (center_x != -1 and center_y != -1)):
                raise cellprofiler_core.setting.ValidationError("Circle coordinates must either both be -1 (default) or both positive",
                                                           self.circle_center)
